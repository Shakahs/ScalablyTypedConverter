...
object Progress {
  ...
  /**
    ...
    */
  final case class State(open: Map[TsIdentLibrary, Int], finished: Set[TsIdentLibrary], failed: Set[TsIdentLibrary]) {
    def on(lib: TsIdentLibrary, event: PhaseListener.Event[LibTsSource]): State =
      event match {
        ...
        case PhaseListener.Blocked(_, _) | PhaseListener.Resumed(_) =>
          this
      }

    private def done(lib: TsIdentLibrary): State =
      open.getOrElse(lib, 0) match {
        case n if n <= 1 => State(open - lib, finished + lib, failed)
        case n           => State(open.updated(lib, n - 1), finished + lib, failed)
      }
    ...
    def render: String = {
      ...
      s"[$bar] $processed/$total packages processed, $pending pending$failures"
    }
  }

  object State {
    val Empty: State = State(Map.empty, Set.empty, Set.empty)
  }
